...
public QueryClientConVarCallback(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	if (IsClientInGame(client) && !IsClientInKickQueue(client))
	{
							 ...
							 */
		{
			...
			PrintToChatAll("\x01[\x05TS\x01] \x03%s\x01 spectated due to \x04c_thirdpersonshoulder\x01 not valid or protected!", sName);
		}
											 ...
											 */
		{
			...
			GetClientName(client, sName, sizeof(sName));
			ChangeClientTeam(client, 1);
			PrintToChatAll("\x01[\x05TS\x01] \x03%s\x01 spectated due to \x04c_thirdpersonshoulder\x01, set at\x05 0\x01 to play!", sName);
		}
	}
}
