...
{
	RegConsoleCmd("witch_glow_info", PrintInfo);
...
public Action:PrintInfo(client, args)
{
	PrintToChat(client, "%s",InfoPlugin);
}
...
CreateWitchPropSpawner(WitchID)
{
	...
	DispatchKeyValueVector(i_Ent[WitchID], "origin", WitchvPos[WitchID]);
	DispatchKeyValueVector(i_Ent[WitchID], "angles", WitchvAvg[WitchID]);
