...
public void OnPluginStart()
{
	...
	g_hCvarTargets =		CreateConVar(	"l4d_target_patch_targets",			"12",				"0=Off. Block these Special Infected from targeting incapped players: 1=Smoker, 2=Boomer, 4=Hunter, 8=Tank, 15=All. Add numbers together.", CVAR_FLAGS );
...
public MRESReturn ChooseVictimPost(int client, Handle hReturn)
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		...
		{
			if(IsPlayerPinned(i)) //ignore player pinned by hunter&smoker
				continue;
	
			if(IsHandingFromLedge(i))  //ignore player handingFromLedge
				continue;	
				
			if(IsIncapacitated(i))  //ignore player incapped
				continue;	
			
			/*
			if(IsPlayerOnVomit(i)) //ignore player get vomited
				continue;
			*/
