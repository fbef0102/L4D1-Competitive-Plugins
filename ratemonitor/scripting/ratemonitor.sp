...
{	
    ...
    if (iIndex > -1) 
    {
        ...
        //               sCounter);
        PrintToChat(client,"\x03Your\x01 netsettings changed from \x04%d\x01/\x04%d\x01/\x05%d\x01 to \x04%d\x01/\x04%d\x01/\x05%d\x01%s", 
                        player[Client_Cmdrate], player[Client_Updaterate], player[Client_Rate], 
                        iCmdRate, iUpdateRate, iRate,
                       sCounter);
    ...
    else
    {
        ...
        PrintToChat(client,"\x03Your\x01 netsettings set to \x04%d\x01/\x04%d\x01/\x05%d\x01",player[Client_Cmdrate], player[Client_Updaterate], player[Client_Rate]);
	}
}
